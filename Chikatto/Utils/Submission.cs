...
namespace Chikatto.Utils
{
    public static class Submission
    {
        ...
        public static async Task<Score> ScoreDataToScore(string[] scoreData)
        {
            ...
            var mods = (LegacyMods) int.Parse(scoreData[13]);
            var score = new Score
            {
                ...
                Mods = mods,
                Completed = scoreData[14] == "True" ? RippleScoreCompleted.Pass : RippleScoreCompleted.Failed,
                PlayMode = (GameMode) int.Parse(scoreData[15]),
                IsRelax = (mods & LegacyMods.Relax) != 0,
                Time = DateTimeOffset.UtcNow.ToUnixTimeSeconds().ToString(),
                ChickenMcNuggetsHash = scoreData[2]
            };

            switch (score.PlayMode)
            {
                case GameMode.Taiko:
                    score.Accuracy = (0.5f * score.Count100 + score.Count300) /
                                     (score.Count300 + score.Count100 + score.CountMiss);
                    break;
                case GameMode.Catch:
                    score.Accuracy = (double)(score.Count50 + score.Count100 + score.Count300) /
                                     (score.Count50 + score.CountGeki + score.CountKatu + score.Count300 + score.Count100 + score.CountMiss);
                    break;
                case GameMode.Mania:
                    score.Accuracy = (score.Count300 * 300f + score.CountGeki * 300f + score.CountKatu * 200f + score.Count100 * 100f + score.Count50 * 50f) 
                                     / ((score.Count300 + score.CountGeki + score.CountKatu + score.Count100 + score.Count50 + score.CountMiss) * 300f) 
                                     * 100f;
                    break;
                
                default:
                case GameMode.Standard:
                    score.Accuracy = (score.Count300 * 300f + score.Count100 * 100f + score.Count50 * 50f) 
